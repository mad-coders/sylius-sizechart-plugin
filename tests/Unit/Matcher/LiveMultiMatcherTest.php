...
use Madcoders\SyliusSizechartPlugin\Matcher\LiveMultiMatcher;
 ...
 * @author Piotr Lewandowski <p.lewandowski@madcoders.pl>
 */
class LiveMultiMatcherTest extends AbstractUnit
{
    /**
     * @test
     */
    function it_returns_all_matches()
    {
        // given
        ...
        $matcher = new LiveMultiMatcher($sizeChartRepository->reveal(), $sizeChartValidator->reveal());

        // when
        $result = $matcher->match($product->reveal());

        // then
        $this->assertEquals([$sizeChartA, $sizeChartB], $result);
    }
